...
echo "Releasing Connect SDK ${SDK_VERSION}"
...
# Now build and publish the Connect SDK and its dependencies
echo "Building and publishing the Connect SDK"
pushd ${THIS_DIR}/../
./gradlew clean 
...
if [[ $DO_GIT_TAG == 1 ]]; then
    ...
    pushd ${MVN_REPO_PATH}
    git add -A .
    git commit -m "Connect SDK + dependencies: ${SDK_VERSION}"
    popd
